...
function CPreviewPaneMessageHeaders()
{
	...
	this._subjCont = null;
	this._switchDetailsCont = null;
	this._switchDetailsObj = null;
...
CPreviewPaneMessageHeaders.prototype = {
	ShowShort: function (showDetailsSwitcher)
	{
		...
		if (showDetailsSwitcher) {
			this._switchDetailsCont.className = 'wm_message_right';
			this._switchDetailsObj.innerHTML = Lang.MessageShowDetails;
		}
		this._showFull = false;
	},
	...
	ShowFull: function ()
	{
		...
		this._switchDetailsCont.className = 'wm_message_right';
		this._switchDetailsObj.innerHTML = Lang.MessageHideDetails;
		this._showFull = true;
	},
	...
	_fillSubject: function (msg)
	{
		if (msg.Importance == PRIORITY_HIGH) {
			this._subjCont.innerHTML = '<span class="wm_importance_img"> </span>' + msg.Subject;
		}
		else {
			this._subjCont.innerHTML = msg.Subject;
		}
		this._subjCont.title = msg.Subject;
	},
	...
	_fillFrom: function (msg, fromDisplay)
	{
		...
		if (WebMail.Settings.AllowContacts && msg.FromAddr.length > 0) {
			...
			fromParts = GetEmailParts(HtmlDecode(msg.FromAddr));
	...
	Clean: function ()
	{
		...
		this._switchDetailsCont.className = 'wm_hide';
	...
	{
		var switchDetailsWidth = this._switchDetailsCont.clientWidth;
		switchDetailsWidth += (switchDetailsWidth == 0) ? 0 : this._spanMargin * 2;
		var maxSubjWidth = inboxWidth - switchDetailsWidth - this._spanMargin * 2 - this._contPadding * 2;
		this._subjCont.style.width = 'auto';
		var subjWidth = this._subjCont.clientWidth;
		if (subjWidth > maxSubjWidth) {
			this._subjCont.style.width = maxSubjWidth + 'px';
		}
		
		var dateWidth = this._shortDateCont.clientWidth;
		dateWidth += (dateWidth == 0) ? 0 : this._spanMargin * 2;
		var addrBookWidth = this._shortAddToABCont.clientWidth;
		var maxFromWidth = inboxWidth - dateWidth - addrBookWidth - this._spanMargin - this._contPadding * 2;
		maxFromWidth = Validator.CorrectNumber(maxFromWidth, 100);
		this._shortFromCont.style.width = 'auto';
		var fromWidth = this._shortFromCont.clientWidth;
		if (fromWidth > maxFromWidth) {
			this._shortFromCont.style.width = maxFromWidth + 'px';
		}

		addrBookWidth = this._addToABCont.clientWidth;
		maxFromWidth = inboxWidth - addrBookWidth - 4 - this._contPadding * 2;
		maxFromWidth = Validator.CorrectNumber(maxFromWidth, 100);
		this._fromCont.style.width = 'auto';
		fromWidth = this._fromCont.clientWidth;
		if (fromWidth > maxFromWidth) {
			this._fromCont.style.width = maxFromWidth + 'px';
		}

		var switcherWidth = this.SwitcherCont.clientWidth;
		switcherWidth += (switcherWidth == 0) ? 0 : this._spanMargin * 2;
	...
	_addClearDiv: function (cont)
	{
		...
		var div = CreateChild(cont, 'div', [['style', styles]]);
	},
	...
	Build: function (parent)
	{
		...
		this._subjCont = CreateChild(div, 'span', [['style', 'font-size: 14px; font-weight: bold;'],
			['class', 'wm_message_left wm_message_resized']]);
		this._switchDetailsCont = CreateChild(div, 'span', [['class', 'wm_hide']]);
		var a = CreateChild(this._switchDetailsCont, 'a', [['href', '#']]);
		...
		this._shortDateCont = CreateChild(div, 'span', [['class', 'wm_message_left']]);
