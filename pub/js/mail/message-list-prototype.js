...
var MessageListPrototype = {
	...
	GetSearchParameters: function ()
	{
		...
		var searchString = this._lookForBigObj.value;
	...
	PlaceSearchData: function (searchFields, lookFor)
	{
		...
		this.SearchFormObj.SetString(lookFor);
	},
	
	FocusSearchForm: function ()
	{
		this._lookForSmallObj.focus();
	},
	...
	{
		this.SearchFormObj.SetString(this._lookForSmallObj.value);
	...
	BuildAdvancedSearchForm: function ()
	{
		...
		var inp = CreateChild(td, 'input', [['type', 'text'], ['maxlength', '255']]);
		this._lookForBigObj = inp;
		inp.className = 'wm_search_input';
		this._toolBar.CreateSearchButton(td, function () {obj.GetSearchResultsFromBig();});
		...
		inp = CreateChild(nobr, 'input', [['type', 'radio'], ['name', 'qsmode' + this.Id], ['id', 'qmode' + this.Id]]);
