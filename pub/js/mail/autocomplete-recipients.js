...
CPopupAutoFilling.prototype =
{
	...
	ClickBody: function (ev)
	{
		...
		if (this._shown) {
			...
			elem = (Browser.Mozilla) ? ev.target : ev.srcElement;
	...
	{
		var div, obj, iCount, i, innerHtml;
		obj = this;
	...
	{
		var lastKeyLength, end;
		lastKeyLength = this.GetKeyword().length;
		end = this._suggestInput.value.length - lastKeyLength;
		if (end > 0) {
			if (lastKeyLength > 0) {
	...
	SelectLine: function (obj)
	{
		...
		if (Browser.IE) {
			textRange = this._suggestInput.createTextRange();
	...
	{
		var key, td, keyword, obj;
		key = Keys.GetCodeFromEvent(ev);
		switch (key) {
			case Keys.Enter:
				if (this._pickPos != -1) {
					td = this._lines[this._pickPos];
					this.SelectLine(td);
				}
				break;
			case Keys.Up:
	...
	CheckRequestKeyword: function (keyword)
	{
		...
		if (keyword.length > 0 && this._keyword != keyword) {
			if (this._requestKeyword.length > 0) {
				reg = new RegExp(this._requestKeyword.PrepareForRegExp(), 'gi');
				res = reg.exec(keyword);
