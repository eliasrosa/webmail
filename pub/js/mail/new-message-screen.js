...
*/		
	...
	GetExternalHeight: function()
	{
		...
		if (x = this._toolBar.GetHeight()) {res += x;}
		if (x = this._headersTbl.offsetHeight) {res += x;}
		if (x = this._plainEditorCont.offsetHeight) {res -= x;}
		if (x = this._modeSwitcherTr.offsetHeight) {res += x;}
		if (x = this._attachmentsDiv.offsetHeight) {res += x;}
		if (!this._flashFileUpload) {
	...
	ResizeBody: function ()
	{
		if (this.isBuilded) {
			...
			var height = GetHeight() - this.GetExternalHeight() - padding;
			if (height < 250) {
				height = 250;
			}
			...
			if (this._flashFileUpload) {
				...
				for (var i = 0; i < msg.Attachments.length; i++) {
					...
					if (att.Inline) {
						...
						progress.setStatus("");
						progress.toggleCancel(true, SwfUploader);
					}
				}
			}
			else {
	...
	_markMsgAsReplied: function (msg)
	{
	...
	ChangePriority: function ()
	{
		...
		switch (pr) {
			case PRIORITY_LOW:this.SetPriority(PRIORITY_NORMAL);break;
			case PRIORITY_NORMAL:this.SetPriority(PRIORITY_HIGH);break;
			case PRIORITY_HIGH:this.SetPriority(PRIORITY_LOW);break;
		}
	},
	...
	Build: function (container, accountsBar, popupMenus)
	{
		...
		if (flashInstalled == FLASH_INSTALLED) {
			...
			var uploadProgress = CreateChild(div, 'div', [['class', 'wm_hide'], ['id', 'fsUploadProgress']]);
			var divButtonPlaceHolder = CreateChild(div, 'div');
			divButtonPlaceHolder.style.marginTop = '10px';
			var spanButtonPlaceHolder = CreateChild(divButtonPlaceHolder, 'span', [['id', 'spanButtonPlaceHolder']]);
