...
CViewMessageScreen.prototype = {
	...
	Fill: function (msg)
	{
		...
		this._sSubjectObj.innerHTML = (HtmlDecode(msg.Subject).length <= 20)
			? '&nbsp;' + msg.Subject + '&nbsp;' : '&nbsp;' + HtmlEncode(HtmlDecode(msg.Subject).substr(0, 20)) + '...&nbsp;';
	...
	CreatePrintLinkFunc: function (link, msg)
	{
		...
		return function () {
			...
			var win = window.open('', 'PopupPrintMessage', 'toolbar=yes,status=no,scrollbars=yes,resizable=yes,width=' + width + ',height=' + height + ',top=' + top + ',left=' + left);
			if (Browser.Mozilla) {win.document.open();};
			win.document.write('<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd" />');
			win.document.write('<html><head><link rel="stylesheet" href="./skins/' + WebMail.Settings.DefSkin + '/styles.css" type="text/css" /></head>');
			win.document.write('<body class="wm_body"><div align="center" class="wm_space_before"><table class="wm_print">');
			win.document.write('<tr><td class="wm_print_title">' + Lang.From + ': </td><td class="wm_print_value">' + msg.FromAddr + '</td></tr>');
			win.document.write('<tr><td class="wm_print_title">' + Lang.To + ': </td><td class="wm_print_value">' + msg.ToAddr + '</td></tr>');
			if (msg.CCAddr.length > 0) {
				win.document.write('<tr><td class="wm_print_title">' + Lang.CC + ': </td><td class="wm_print_value">' + msg.CCAddr + '</td></tr>');
			}
			win.document.write('<tr><td class="wm_print_title">' + Lang.Date + ': </td><td class="wm_print_value">' + msg.Date + '</td></tr>');
			win.document.write('<tr><td class="wm_print_title">' + Lang.Subject + ': </td><td class="wm_print_value">' + msg.Subject + '</td></tr>');
			var attArray = Array();
			...
			if (attStr.length > 0) {
				win.document.write('<tr><td class="wm_print_title">' + Lang.Attachments + ': </td><td class="wm_print_value">' + attStr + '</td></tr>');
			}

			var messageBody = (msg.HasHtml) ? msg.HtmlBody : msg.PlainBody;
			win.document.write('<tr><td colspan="2" class="wm_print_body"><div class="wm_space_before">' + messageBody + '</div></td></tr></table></div></body></html>');
	...
	_buildToolBar: function (container, PopupMenus)
	{
		...
		toolBar.AddItem(TOOLBAR_NEW_MESSAGE, function () {
			SetHistoryHandler(
				{
