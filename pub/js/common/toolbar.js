...
CToolButton.prototype = {
	...
	{
		var div, titleLangField, span;
		div = CreateChild(container, 'span', [['class', 'wm_toolbar_icon'],
			['style', 'background-position: -' + desc.x * X_ICON_SHIFT + 'px -' + desc.y * Y_ICON_SHIFT + 'px']]);
		div.innerHTML = '&nbsp;';
		titleLangField = (desc.titleLangField) ? desc.titleLangField : desc.langField;
		if (titleLangField) {
		...
		if (desc.langField) {
			span = CreateChild(container, 'span');
...
function CToolBar(parent, viewMode)
{
	...
	this._descriptions[TOOLBAR_SENSIVITY_NOTHING] = { langField: 'SensivityNothingMenu', x: 7, y: 5, iconClassName: 'wm_hide' };
	this._descriptions[TOOLBAR_SENSIVITY_CONFIDENTIAL] = { langField: 'SensivityConfidentialMenu', x: 7, y: 5, iconClassName: 'wm_hide' };
	this._descriptions[TOOLBAR_SENSIVITY_PRIVATE] = { langField: 'SensivityPrivateMenu', x: 7, y: 5, iconClassName: 'wm_hide' };
	this._descriptions[TOOLBAR_SENSIVITY_PERSONAL] = { langField: 'SensivityPersonalMenu', x: 7, y: 5, iconClassName: 'wm_hide' };
...
CToolBar.prototype = {
	_buildCurve: function ()
	{
		...
		var div = CreateChild(this.table, 'div', [['class', 'wm_toolbar_curve_inner ' + leftClassName]]);
		div = CreateChild(this.table, 'div', [['class', 'wm_toolbar_curve_outer ' + leftClassName]]);
		div = CreateChild(this.table, 'div', [['class', 'wm_toolbar_curve_inner ' + rightClassName]]);
		div = CreateChild(this.table, 'div', [['class', 'wm_toolbar_curve_outer ' + rightClassName]]);
	},
	...
	AddItem: function (itemId, clickHandler, show) {
		var div, button;
		div = CreateChild(this._container, 'span');
		button = new CToolButton(div, this._descriptions[itemId], clickHandler, show);
	...
	AddMarkItem: function (popupMenus, show) {
		...
		var button, div, markFunc;
		for (var i = TOOLBAR_MARK_UNREAD; i <= TOOLBAR_MARK_ALL_UNREAD; i++) {
			div = CreateChild(markMenu, 'div');
			markFunc = CreateToolBarItemClick(i);
			button = new CToolButton(div, this._descriptions[i], markFunc, true);
			this._buttons.push(button);
			switch (i) {
			case TOOLBAR_UNFLAG:
				div = CreateChild(markMenu, 'div');
