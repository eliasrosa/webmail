			...
			case TYPE_UPDATE:
				switch (Data.Value) {
					...
					case 'send_message':
						...
						var screen = this.Screens[this.ListScreenId];
						if (screen) {
							this.DataSource.Cache.ClearMessageList(screen.SentId, screen.SentFullName);
							this.DataSource.Cache.ClearMessageList(screen.DraftsId, screen.DraftsFullName);
					...
					case 'save_message':
						...
						if (screen) {
							this.DataSource.Cache.ClearMessageList(screen.DraftsId, screen.DraftsFullName);
							if (typeof(screen.SetMessageId) == 'function') screen.SetMessageId(Data.Id, Data.Uid);
						}
						this.Screens[this.ScreenId].isSavedOrSent = true;
						if (this.Accounts.CurrMailProtocol != POP3_PROTOCOL) {
							...
							this.DataSource.Cache.ClearMessage(Data.Id, Data.Uid, screen.DraftsId, screen.DraftsFullName, '');
						}
						if (screen) {
							GetMessageListHandler(REDRAW_NOTHING, null, screen.DraftsId, screen.DraftsFullName, 
							    screen._sortField, screen._sortOrder, screen._page, screen._lookForStr, screen._searchMode, true);
