		...
		else
		{
			if (USE_DB)
			{
				...
				else
				{
					...
					if (ConvertUtils::DecodePassword($_loginArray[1], $_newAccount) == $_mailIncPass)
					{
						$_SESSION[ACCOUNT_ID] = $_loginArray[0];
						$_SESSION[USER_ID] = $_loginArray[3];
						...
						if ($_useLangUpdate)
						{
							$_newAccount->UpdateDefaultLanguage();
						}

						$_log->WriteEvent('User login', $_account);

						$_newprocessor = new MailProcessor($_newAccount);
						if ($_newAccount->MailProtocol == MAILPROTOCOL_IMAP4 && $_newAccount->ImapQuota === 1)
						{
							if ($_newprocessor->MailStorage->Connect(true))
							{
								$quota = $_newprocessor->GetQuota();
								if ($quota !== false && $quota !== $_newAccount->MailboxLimit)
								{
									$_newAccount->MailboxLimit = GetGoodBigInt($quota);
									$_newAccount->UpdateMailBoxLimit();
								}
							}
						}
	...
	function AfterLoginAction(&$_account, &$_newprocessor, $_settings)
	{
				...
				($_account->MailProtocol == MAILPROTOCOL_POP3 || $_account->MailProtocol == MAILPROTOCOL_IMAP4))
		{
			if ($_newprocessor->MailStorage->Connect(true))
			{
				...
				if ($folders)
				{
					self::checkSystemFolderAndCreate($_newprocessor, $_account, $folders, FOLDERTYPE_Spam, FOLDERNAME_Spam);
					self::checkSystemFolderAndCreate($_newprocessor, $_account, $folders, FOLDERTYPE_Trash, FOLDERNAME_Trash);
				}
			}
		}
	}

	function checkSystemFolderAndCreate(&$_newprocessor, &$_account, &$folders, $folderType, $folderName)
	{
