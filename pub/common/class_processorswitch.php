	...
	function DoUpdateFolders()
	{
		...
		for ($_key = count($_foldersNode->Children) - 1; $_key >= 0; $_key--)
		{
			...
			if ($_folder->Name != $_newFolderName)
			{
				...
				$_result &= ($_account->IsDemo) ? true : $_processor->RenameFolder($_folder, $_newFolderName, $_account->Delimiter);
		...
		if (CSmtp::SendMail($_settings, $_account, $_message, null, null))
		{
			...
			if ($_processor->DbStorage->Connect())
			{
				...
				if ($_needToDelete)
				{
					...
					if ($_draftsFolder)
					{
						if (!$_processor->SaveMessage($_message, $_folder, $_draftsFolder))
						{
							$_needToDelete = false;
						}
					}
